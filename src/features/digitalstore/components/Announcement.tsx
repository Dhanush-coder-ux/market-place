...
export default function Announcement() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="rounded-2xl p-1.5 w-44 shadow-xl border-slate-100">
                {(Object.keys(TYPE_CONFIG) as Array<keyof typeof TYPE_CONFIG>).map((t) => {
                  ...
                  return (
                    <DropdownMenuItem
                      key={t}
                      onClick={() => setType(t)}
                      className={`flex items-center gap-2 rounded-xl px-3 py-2 text-[12px] font-semibold cursor-pointer transition-colors ${type === t ? `${c.bg} ${c.color}` : "text-slate-600 hover:bg-slate-50"}`}
                    >
            ...
            <Popover>
              <PopoverTrigger asChild>
                <button className={cn(
                  "inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold border transition-all outline-none",
                  date
          ...
          <Popover open={historyOpen} onOpenChange={setHistoryOpen}>
            <PopoverTrigger asChild>
              <button className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-semibold border border-slate-200 text-slate-500 hover:border-indigo-300 hover:text-indigo-600 hover:bg-indigo-50 transition-all outline-none">
            ...
            <PopoverContent className="w-80 p-0 rounded-2xl shadow-xl border-slate-100" align="end">
              <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                <p className="text-[13px] font-black text-slate-800">Past Announcements</p>
                <button onClick={() => setHistoryOpen(false)} className="w-6 h-6 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 transition-colors">
                  <X size={13} strokeWidth={2.5} />
                </button>
              </div>
              <div className="max-h-[280px] overflow-y-auto p-2.5 space-y-1.5">
                {PREVIOUS.map((prev) => {
                  const pc = TYPE_CONFIG[prev.type as keyof typeof TYPE_CONFIG];
                  return (
                    <button
                      key={prev.id}
                      onClick={() => { setAnnouncement(prev.text); setType(prev.type as keyof typeof TYPE_CONFIG); setHistoryOpen(false); }}
                      className="w-full text-left p-3 rounded-xl border border-transparent hover:border-indigo-100 hover:bg-indigo-50/40 transition-all group"
                    >
                      ...
                      <p className="text-[12px] text-slate-600 line-clamp-2 group-hover:text-slate-900 transition-colors">{prev.text}</p>
                    </button>
                  );
                })}
              </div>
      ...
      {announcement && (
        <div className={`mt-3 flex items-start gap-3 px-4 py-3 rounded-2xl border transition-all duration-300 ${config.bg} ${config.ring} ring-1`}>
