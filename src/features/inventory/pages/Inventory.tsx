...
export interface Product {
  ...
  batchTracking?: boolean; 
  batches?: Batch[]; 
  originalRecord?: InventoryRecord; // Keep reference for the Drawer
}
...
// Adapter to transform flat API records into the structured format required by the UI
const adaptRecordToProduct = (record: InventoryRecord): Product => {
  return {
    id: record.id,
    name: String(record.datas?.name ?? record.barcode ?? "Unnamed Product"),
    sku: record.barcode,
    description: record.datas?.description ?? "",
    buyPrice: record.buy_price ?? 0,
    sellPrice: record.sell_price ?? 0,
    stock: record.stocks ?? 0,
    hasVariants: !!record.datas?.variants?.length,
    variants: record.datas?.variants, 
    batchTracking: !!record.datas?.batches?.length,
    batches: record.datas?.batches,
...
const Inventory = () => {
  ...
  useEffect(() => {
    ...
    getData(ENDPOINTS.INVENTORIES, params).then((res) => {
      if (res) {
        ...
        const formattedData = rawData.map(adaptRecordToProduct);
  ...
  const toggleSelection = (e: React.ChangeEvent<HTMLInputElement> | null, id: string) => {
  ...
  return (
    <div className="space-y-6 text-slate-800 mx-auto p-4 sm:p-6 lg:p-8">
      
      {/* Header Area */}
      <div className="flex flex-col gap-4">
        <InventoryHeader
          searchValue={searchQuery}
          lowestStockValue={products.filter((p) => isLowStock(p.stock)).length}
      ...
      {/* Bulk Action Bar with Framer Motion */}
      <AnimatePresence>
        {selectedRows.size > 0 && (
          <motion.div
                ...
                <tr className="bg-slate-50/80 border-b border-slate-200/80 text-xs uppercase tracking-wider text-slate-500 font-semibold">
                  <th className="p-4 w-14 text-center">
                    <input 
                      ...
                      onChange={(e) => {
                        if (e.target.checked) setSelectedRows(new Set(products.map((i) => i.id)));
      ...
      <Drawer isOpen={isOpen} onClose={() => setIsOpen(false)} title="Inventory Details">
        <InventoryDetailContent item={selectedItem && 'originalRecord' in selectedItem ? (selectedItem.originalRecord ?? null) : null} />
      </Drawer>
    </div>
  );
};
