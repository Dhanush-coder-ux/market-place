      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white/80 sticky top-0 z-20 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white shadow-lg ${isPositive ? 'bg-[var(--mv-purchase-bg)] shadow-[var(--mv-purchase-bd)]' : 'bg-[var(--mv-preturn-bg)] shadow-[var(--mv-preturn-bd)]'}`}>
        ...
        <div className="flex-1 p-6 space-y-8">
          ...
          {/* Main Impact Hero */}
          <div className={`rounded-lg border-2 p-6 text-center space-y-4 shadow-sm transition-all ${isPositive ? 'bg-[var(--mv-purchase-bg)] border-[var(--mv-purchase-bd)]' : 'bg-[var(--mv-preturn-bg)] border-[var(--mv-preturn-bd)]'}`}>
              ...
              <>
                ...
                <div className="relative pl-6 before:absolute before:left-[11px] before:top-8 before:bottom-0 before:w-0.5 before:bg-slate-100">
                  <div className="relative group mb-4">
                    ...
                    <div className="bg-blue-50/40 border border-[var(--mv-sales-bd)] rounded-lg p-4 transition-all hover:bg-[var(--mv-sales-bg)] hover:shadow-md hover:shadow-blue-500/5">
                      <div className="flex items-center gap-2 text-[var(--mv-sales-tx)] font-black text-[10px]   mb-1">
                  ...
                  {movement.variant && (
                    <div className="relative group mb-4">
                      ...
                      <div className="bg-violet-50/40 border border-[var(--at-variant-bd)] rounded-lg p-4 ml-2 transition-all hover:bg-[var(--mv-sales-bg)] hover:shadow-md hover:shadow-violet-500/5">
                        <div className="flex items-center gap-2 text-[var(--at-variant-tx)] font-black text-[10px]   mb-1">
                  ...
                  {movement.batch && (
                    <div className="relative group mb-4">
                      ...
                      <div className="bg-amber-50/40 border border-[var(--at-batch-bd)] rounded-lg p-4 ml-4 transition-all hover:bg-[var(--mv-sales-bg)] hover:shadow-md hover:shadow-amber-500/5">
                        <div className="flex items-center gap-2 text-[var(--at-batch-tx)] font-black text-[10px]   mb-1">
                        ...
                        {(movement.expiry_date || movement.manufacturing_date) && (
                          <div className="space-y-1.5 border-t border-[var(--at-batch-bd)] pt-2 mt-2">
                            {movement.manufacturing_date && (
                              <div className="flex justify-between items-center text-[10px]">
                                <span className="text-[var(--at-batch-tx)]/70 font-bold  tracking-tight">MFG Date</span>
                            ...
                            {movement.expiry_date && (
                              <>
                                <div className="flex justify-between items-center text-[10px]">
                                  <span className="text-[var(--at-batch-tx)]/70 font-bold  tracking-tight">EXP Date</span>
                                ...
                                <div className="flex justify-between items-center bg-white/50 rounded-lg px-2 py-1 mt-1">
                                  ...
                                  <span className="text-[10px] font-black text-[var(--ps-cancel-tx)] tabular-nums">
                                    {(() => {
                  ...
                  {movement.serial_numbers && movement.serial_numbers.length > 0 && (
                    <div className="relative group">
                      ...
                      <div className="bg-emerald-50/30 border border-[var(--ps-completed-bd)] rounded-lg p-4 ml-6 transition-all hover:bg-[var(--mv-sales-bg)] hover:shadow-md hover:shadow-emerald-500/5">
                        <div className="flex items-center gap-2 text-[var(--ps-completed-tx)] font-black text-[10px]   mb-3">
          ...
          {/* Notes Card */}
          {movement.notes && (
            <div className="bg-[var(--mv-sales-bg)] rounded-lg p-4 border border-[var(--mv-sales-bd)]/50">
              <div className="flex items-center gap-2 text-[var(--mv-sales-tx)] font-bold text-[10px]   mb-2">
...
export default function StockMovementPage() {
    ...
    if (selectedMovements.size > 1) {
      setBottomActions(
        <div className="flex items-center justify-between w-full animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-md bg-[var(--mv-sales-bg)] text-white flex items-center justify-center text-[10px] font-black shrink-0">
              <Activity size={14} />
            </div>
            <div>
  ...
  const SortBtn = ({ field, label, align = "left" }: { field: "date" | "qty", label: string, align?: "left" | "right" }) => (
    <button onClick={() => toggleSort(field)} className={`flex items-center gap-1 text-slate-500 hover:text-[var(--mv-sales-tx)] transition-colors font-semibold group w-full ${align === "right" ? "justify-end" : "justify-start"}`}>
      <span>{label}</span>
      <span className={`transition-opacity ${sortField === field ? "opacity-100 text-[var(--mv-sales-tx)]" : "opacity-0 group-hover:opacity-40"}`}>
              ...
              <tr className="text-slate-400 text-[10px] font-bold tracking-[0.15em]">
                <th className="px-4 py-3 w-10 text-center border-r border-slate-100">
                  <input
                    ...
                    className="rounded border-slate-350 text-[var(--mv-sales-tx)] focus:ring-blue-500/20 cursor-pointer"
                  />
                </th>
                ...
                return (
                  <React.Fragment key={rowKey}>
                    <tr
                      ...
                      className={`group transition-all cursor-default border-b border-slate-100 last:border-b-0 ${selectedMovements.has(m.id) ? "bg-[var(--mv-sales-bg)] border-l-[var(--mv-sales-dot)]" : "hover:bg-blue-50/30 even:bg-slate-50/20"}`}
                    >
                        ...
                        if (key === "notes" && displayValue !== "—") {
                          return (
                            <td key={key} className="px-4 py-3 align-middle border-r border-slate-100 last:border-r-0">
                              <p className="text-[12px] font-bold text-slate-600 tracking-tight whitespace-normal break-words">
                                {displayValue}
                              </p>
                            </td>
                          );
                      ...
                      <td className="px-4 py-3 text-center align-middle w-14 whitespace-nowrap sticky right-0 bg-white group-hover:bg-slate-50 border-l border-slate-200 z-10 shadow-[-8px_0_12px_-4px_rgba(0,0,0,0.08)] transition-colors" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="text-emerald-500 hover:text-[var(--ps-completed-tx)] transition-colors p-1"
                          title="View Details"
                        >
                          <Eye size={15} />
                        </button>
