        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 pf-mono mt-0.5">{comboLabel} · {combo.sku}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
...
const VariantMatrixTable: React.FC<VariantMatrixTableProps> = ({
  ...
  const regenAllSKUs = (baseSKU: string) => {
    if (!baseSKU) return;
    onChange(combinations.map((c, i) => ({
      ...c,
      sku: `${baseSKU}-${Object.values(c.attributes).map(v => v.slice(0, 3).toUpperCase()).join("-")}-${String(i + 1).padStart(2, "0")}`,
    })));
  };
      ...
      <div className="flex items-center justify-between mb-3 gap-2 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            ...
            />
            <button type="button" onClick={() => regenAllSKUs(skuBase)}
              disabled={!skuBase}
...
const generateCombinations = (
  ...
  return combos.map(combo => {
    ...
    const skuSuffix = combo.map(v => v.slice(0, 3).toUpperCase()).join("-");
    return {
      id: uid(),
      attributes,
      sku: skuSuffix,
...
const ProductForm: React.FC<ProductFormProps> = ({ initialData = {}, isLoading: externalLoading = false }) => {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    costPrice: (initialData.cost_price as string) || "",
    sellingPrice: (initialData.selling_price as string) || "",
    ...
    openingStock: (initialData.opening_stock as string) || "",
    reorderPoint: (initialData.reorder_point as string) || "5",
    maxStock: (initialData.max_stock as string) || "",
  ...
  const totalStock = useMemo(() => {
    if (!form.hasVariants) return Number(form.openingStock) || 0;
    return combinations.reduce((s, c) => s + (Number(c.stock) || 0), 0);
  }, [form.hasVariants, form.openingStock, combinations]);
          ...
          <div className="flex-1 w-full space-y-5">
            ...
            <div className="pf-card bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <InputField
                  label="SKU / Barcode" name="sku" required
            ...
            <div className="pf-card bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <InputField
                  label="Cost Price" name="costPrice" required
                  type="number" leftEl="₹"
                  value={form.costPrice} onChange={handleChange}
                  placeholder="0.00"
                />
                <InputField
                  label="Selling Price" name="sellingPrice" required
                  type="number" leftEl="₹"
            ...
            <div className="pf-card bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
              ...
              <div className="mb-5">
                <Toggle
                  active={form.hasVariants}
                  onChange={() => {
                    setForm(p => ({ ...p, hasVariants: !p.hasVariants }));
                    if (!form.hasVariants) setCombinations([]);
                  }}
                  label="This product has variants"
                  hint="Enable to manage multiple SKUs per product (e.g. iPhone 15 in 128GB/Black)"
                />
              </div>
                  ...
                  {combinations.length > 0 && (
                    <div>
                      ...
                      <VariantMatrixTable
                        ...
                        variantTypes={variantTypes}
                        basePriceStr={form.sellingPrice}
              ...
              <div className="pf-card bg-white border border-slate-200 rounded-2xl p-6 shadow-sm pf-section-enter">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                  <InputField
                    ...
                    type="number" value={form.openingStock} onChange={handleChange}
                    placeholder="0"
                  />
                  <InputField
                    label="Reorder Point" name="reorderPoint" required
                    type="number" value={form.reorderPoint} onChange={handleChange}
                    placeholder="5"
                  />
                  <InputField
                    label="Max Stock" name="maxStock"
                    type="number" value={form.maxStock} onChange={handleChange}
                    placeholder="0"
                  />
              ...
              <div className="pf-card bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                ...
                <div className="space-y-0 text-sm divide-y divide-slate-100">
                  {[
                    ...
                    { label: "Cost",     value: form.costPrice ? `₹${Number(form.costPrice).toLocaleString()}` : "—", mono: true },
                    { label: "Price",    value: form.sellingPrice ? `₹${Number(form.sellingPrice).toLocaleString()}` : "—", mono: true },
                    ...
                    { label: "Variants", value: form.hasVariants ? `${combinations.length} combos` : "None" },
                ...
                <div className="mt-3 pt-3 border-t border-slate-100">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${form.isActive ? "pf-status-active" : "pf-status-draft"}`}>
                ...
                <div className="mt-4 flex items-start gap-2.5 bg-blue-50/70 border border-blue-100 rounded-xl px-3.5 py-3">
                  ...
                  <p className="text-[11px] text-blue-700 leading-relaxed">
                    ...
                    {form.hasVariants && " Each variant must have a SKU."}
                  </p>
                </div>
              </div>
