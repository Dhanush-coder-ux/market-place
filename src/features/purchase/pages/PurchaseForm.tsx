...
import { inventoryApi } from "@/services/api/inventory";type PaymentMethod = "Cash" | "UPI" | "Card" | "Bank";
...
const PurchaseForm = () => {
  ...
  const [products, setProducts] = useState<ProductItem[]>([
    {
      ...
      unit: "Piece", taxGst: 18, storageLoc: "", reorderPoint: "", expiryDate: "", manufacturingDate: "", batchTracking: false, batchNum: "", sku: "", variant: "", size: ""
    }
  ]);
  ...
  // --- SUBMIT HANDLER (Payload Fixes Applied) ---
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/70">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  />
                  {supplierDetails && (
                    <div className="mt-2 p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs flex flex-col gap-1 shadow-sm transition-all text-slate-600">
                      <strong className="text-slate-800 text-[13px]">{supplierDetails.name || purchaseDetails.supplier}</strong>
                      {supplierDetails.email && <span>📧 {supplierDetails.email}</span>}
                      {supplierDetails.mobile_number && <span>📞 {supplierDetails.mobile_number}</span>}
                    </div>
                  )}
                </div>
                {/* ------------------------------- */}
                            ...
                            <SearchSelect
                              ...
                              onChange={(val: string, opt: any) => {
                                if (!opt) {
                                  ...
                                  updated[index].unit = opt.unit ?? "Piece";
                                  updated[index].category = opt.category ?? "";
                                  setProducts(updated);
                                }
                              }}
